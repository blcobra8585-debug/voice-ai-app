...
function EffectButton({ effect, isProcessing, onPress, disabled, index }: {
  effect: EffectConfig; isProcessing: boolean; onPress: () => void; disabled: boolean; index: number;
}) {
  ...
  React.useEffect(() => {
    if (isProcessing) {
      glowOpacity.value = withRepeat(withSequence(withTiming(0.7, { duration: 350 }), withTiming(0.12, { duration: 350 })), -1, true);
    } else {
  ...
  return (
    <Animated.View entering={FadeIn.delay(index * 30).duration(250)} style={styles.btnWrap}>
      ...
      >
        ...
        <Text style={[styles.effectLabel, { color: isProcessing ? effect.color : colors.foreground }]} numberOfLines={1}>{effect.label}</Text>
        <Text style={[styles.effectDesc, { color: colors.mutedForeground }]} numberOfLines={1}>{effect.description}</Text>
      </Pressable>
    </Animated.View>
  );
}
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.grid}>
          {aiEffects.map((effect, i) => (
            <EffectButton key={effect.id} effect={effect} index={i}
              isProcessing={processingEffect === effect.id}
              onPress={() => onSelect(effect.id)} disabled={!!disabled} />
          ))}
        </View>
        <Text style={[styles.groupLabel, { color: colors.mutedForeground, marginTop: 10 }]}>⚡ LOCAL EFFECTS</Text>
        <View style={styles.grid}>
          {localEffects.map((effect, i) => (
            <EffectButton key={effect.id} effect={effect} index={aiEffects.length + i}
              isProcessing={processingEffect === effect.id}
              onPress={() => onSelect(effect.id)} disabled={!!disabled} />
          ))}
        </View>
...
const styles = StyleSheet.create({
  ...
  effectBtn: { width: BTN_SIZE, borderRadius: 14, padding: 9, alignItems: "center", gap: 5, overflow: "hidden", borderWidth: 1 },
  aiBadge: { position: "absolute", top: 5, right: 5, borderRadius: 5, borderWidth: 1, paddingHorizontal: 4, paddingVertical: 1 },
