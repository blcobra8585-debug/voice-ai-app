...
export function VoiceProvider({ children }: { children: React.ReactNode }) {
  ...
  const setAutoStopSeconds = useCallback((s: number) => {
    setAutoStopSecondsState(s); saveSettings({ autoStopSeconds: s });
  }, []);

  const setPitchOffset = useCallback((p: number) => {
    setPitchOffsetState(p); saveSettings({ pitchOffset: p });
  }, []);

  const setVolumeBoost = useCallback((v: number) => {
    setVolumeBoostState(v); saveSettings({ volumeBoost: v });
  }, []);
